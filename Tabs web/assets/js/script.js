...
function heart() {
  ...
  heartContainers.forEach(async (container) => {
    ...
    try {
      const initialLikes = await fetchLikes(heartId);
      heartCountElement.textContent = initialLikes;
      heartCheckbox.checked = initialLikes > 0;

      heartCheckbox.addEventListener("change", async function () {
        if (this.checked) {
          const updatedLikes = await updateLikes(heartId, "like");
          if (updatedLikes !== null) heartCountElement.textContent = updatedLikes;
        } else {
          const updatedLikes = await updateLikes(heartId, "unlike");
          if (updatedLikes !== null) heartCountElement.textContent = updatedLikes;
        }
      });
    } catch (error) {
      console.error("Error in heart function:", error);
    }
  });
}
